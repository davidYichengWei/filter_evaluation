...
rotateLeft(uint64_t n, unsigned int c) {
  const unsigned int mask = (CHAR_BIT * sizeof(n) - 1);
  c &= mask;
  return (n << c) | (n >> ((-c) & mask));
}

static inline int word_rank(uint64_t val) { return __builtin_popcountll(val); }
...
static inline uint64_t lookup_128(uint64_t *vector, uint64_t rank) {
  ...
  uint64_t lower_return =
      _pdep_u64(one[rank], lower_word) >> rank << sizeof(__uint128_t);
...
static inline void update_tags_512(vqf_block *restrict block, uint8_t index,
                                   uint8_t tag) {
  index -= 16;
...
vqf_filter *vqf_init(uint64_t nslots) {
  ...
  uint64_t total_blocks =
      (nslots + QUQU_SLOTS_PER_BLOCK) / QUQU_SLOTS_PER_BLOCK;
...
static inline bool check_tags(vqf_filter *restrict filter, uint64_t tag,
  ...
  __m256i block =
      _mm256_loadu_si256(reinterpret_cast<__m256i *>(&filter->blocks[index]));
  __m256i result1t = _mm256_cmpeq_epi8(bcast, block);
  __mmask32 result1 = _mm256_movemask_epi8(result1t);
  block = _mm256_loadu_si256(
      reinterpret_cast<__m256i *>((uint8_t *)&filter->blocks[index] + 32));
  ...
  uint64_t start = offset != 0
                       ? lookup_128(filter->blocks[index].md, offset - 1)
                       : one[0] << 2 * sizeof(uint64_t);
...
bool vqf_is_present(vqf_filter *restrict filter, uint64_t h) {
  ...
  return check_tags(filter, tag, block_index) ||
         check_tags(filter, tag, alt_block_index);
}
...
bool vqf_insert(vqf_filter *restrict filter, uint64_t h) {
  ...
  if (block_free < QUQU_CHECK_ALT &&
    ...
    uint64_t *alt_block_md =
        blocks[alt_block_index / QUQU_BUCKETS_PER_BLOCK].md;
